...
// Định nghĩa quan hệ (Thêm constraints: false để không bị lỗi khóa ngoại với các tin nhắn cũ có accountId = 0)
...
export const index = async (req, res, next) => {
    try {
        ...
        const roomCode = `client_${userId}`; // ID phòng mặc định của khách

        // 1. Kiểm tra hoặc Tạo mới RoomChat cho Khách hàng này
        const [room] = await RoomChat.findOrCreate({
            where: { code: roomCode },
            defaults: {
                code: roomCode,
                user_id: userId,
        ...
        // 3. Lấy lịch sử tin nhắn của phòng này
        ...
        res.json({
            code: "success",
            room_id: room.code, // Trả về room.code để React biết mà tham gia phòng
            messages: messages
        });
    } catch (error) {
        next(error);
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("CLIENT_SEND_MESSAGE", async (data) => {
            try {
                ...
                const room = await RoomChat.findOne({ where: { code: data.room_id } });
                if (!room) {
                    console.error("❌ Không tìm thấy phòng chat:", data.room_id);
                    return;
                }
                ...
                await RoomChat.update(updateData, { where: { id: room.id } });
        ...
        socket.on("CLIENT_SEEN", async (data) => {
            ...
            try {
                if (data.seenBy === 'User') {
                    await RoomChat.update({ unreadCountUser: 0 }, { where: { code: data.room_id } });
                } else {
                    await RoomChat.update({ unreadCountAdmin: 0 }, { where: { code: data.room_id } });
                }
