...
export const login = async (req, res) => {
  try {
    ...
    // Nếu tài khoản có vai trò admin/staff → load thêm thông tin role
    let role = null;
    if (user.roleId) {
      role = await Role.findOne({
        where: { id: user.roleId, deleted: false }
      });
      if (!role) {
        return res.status(403).json({
          code: "error",
          message: "Vai trò của bạn không hợp lệ hoặc đã bị xóa"
        });
      }
    }
    ...
    return res.json({
      ...
      user: {
        ...
        phone: user.phone,
        avatar: user.avatar,
        roleId: user.roleId, // null nếu là người dùng thường
      },
      role: role
...
// [PATCH] /api/client/auth/profile - Cập nhật thông tin cá nhân khách hàng
