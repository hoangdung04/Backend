...
export const createPost = async (req, res, next) => {
  try {
    const { title, description, content, tags, status, tourIds } = req.body;

    // Image đã được upload middleware xử lý
    const image = req.body.image
      ? (Array.isArray(req.body.image) ? req.body.image[0] : req.body.image)
      : "";
