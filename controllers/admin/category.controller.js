...
export const index = async (req, res) => {
  try {
    ...
    for (const cat of categories) {
      if (cat.parentId) {
        const parent = await Category.findOne({ where: { id: cat.parentId, deleted: false }, raw: true });
        cat.parentTitle = parent ? parent.title : "";
      } else {
        cat.parentTitle = "";
      }
    }
...
export const editPatch = async (req, res) => {
  try {
    ...
    if (req.body.parentId !== undefined) {
      updateData.parentId = req.body.parentId ? parseInt(req.body.parentId) : null;
    }
